...
def load_and_process(db_file, up_file, wip_reduction_pct):
    try:
        ...
        # 📌 อัปเดตจำนวนเครื่องจักรทั้งหมดตามตารางล่าสุดของคุณ
        machine_updates = {
            "INJECT 1.6 L 500x600 HOT EM": 7,
            "INJ 2.5 L 500x600 200T EM": 2,
            "INJECT 1.6 L 500x600 COOL RUNNER VA": 9,
            "INJECT 1.6 L 500x600 COOL RUNNER TMA": 5,
            "INJECT 1.6 L 600x600 COOL RUNNER VC": 10,
            "INJECT 1.6 L 600x600 COOL RUNNER (Daylight 800-870 mm.)": 7,
            "INJECT 2.5 L 700x700 COOL RUNNER VJ": 5,
            "INJ 350 T 700x700 COOL RUNNER": 3,
            "INJECT 0.8 L 450x510": 9,
            "PRESS 510X510": 4,
            "VACUUM 700x700": 2,
            "VACUUM 510x510": 2,
            "INJECT 300T": 1,
        ...
        for m_name, m_count in machine_updates.items():
            ...
            if not matched:
                new_row = pd.DataFrame([{
                    'Machine Type': m_name, 
                    'จำนวนเครื่องทั้งหมด': m_count, 
                    'จำนวนเครื่องที่ให้ใช้ได้': m_count
                }])
