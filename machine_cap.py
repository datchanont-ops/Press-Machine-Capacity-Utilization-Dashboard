...
with tab_avail:
    ...
    with col_toggle:
        ...
        simulate_max = st.toggle("🚀 จำลองสถานการณ์: เดินเครื่องเต็มสูบ (ตั้งเปิดใช้ = มีทั้งหมด ทุกเครื่อง)", value=False)
    
    # คำนวณหาค่าแบบสมมติ (ถ้าปุ่มเปิดอยู่ จะบังคับใช้ Total Machines มาเป็นฐานคำนวณแทน)
    ...
    mock_util = np.where(mock_avail_hrs > 0, (cfg['Req_Hours'] / mock_avail_hrs) * 100.0, 0.0)
